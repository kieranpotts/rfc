...
An RFC earns its number only once it is decided and merged. While an RFC is
still open — `#proposed` or `#accepted` — it lives at `rfc/<category>/<slug>/`
with no number, and does not appear here.

The **decision date** is the date the RFC was decided — its approval date for
implemented RFCs, its rejection date for rejected ones — as recorded in the RFC
document.
