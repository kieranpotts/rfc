...
permanently in `rfc/` as the record of the decision and its rationale, so the
same ground is not needlessly covered again later.
...
The RFC MUST currently be `PROPOSED`. Confirm _all_ of the following before
rejecting. If any is unmet, report it and pause.
...
- [`AGENTS.md`](../../../AGENTS.md): The full RFC lifecycle, rejection path, and
  immutability rules.
